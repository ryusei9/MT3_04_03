...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	while (Novice::ProcessMessage() == 0) {
		...
		ImGui::SliderFloat("length", &conicalPendulum.length,0.1f,10.0f);
		...
		// 振り子運動開始
		if (isSwingStart) {
			ConicalPendulumSwing(conicalPendulum, ball);
		}
		///
		/// ↑更新処理ここまで
		///
		...
		DrawConicalPendulum(conicalPendulum, ball, worldViewProjectionMatrix, viewPortMatrix);
		///
		/// ↑描画処理ここまで
		///
